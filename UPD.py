...
def get_tokens(uid):
    ...
    if not doc:
        col_tokens.insert_one(
            {"_id": str(uid), "tokens": TOKEN_NEW_USER, "joined": time.time()}
        )
...
def set_tokens(uid, amount):
    col_tokens.update_one(
        {"_id": str(uid)}, {"$set": {"tokens": max(0, amount)}}, upsert=True
    )
...
def save_version_history(versions):
    col_history.update_one(
        {"_id": "history"}, {"$set": {"versions": versions}}, upsert=True
    )
...
mistral_client = (
    OpenAI(api_key=MISTRAL_KEY, base_url="https://api.mistral.ai/v1")
    if MISTRAL_KEY
    else None
)
...
def needs_web_search_enhanced(prompt):
    ...
    if backtick_content:
        ...
        has_how_pattern = bool(
            re.search(
                r"как.{0,30}(правильно|делать|использовать|работает|написать|сделать)",
                p,
            )
        )
...
MODELS_INFO = {
    ...
    "gemini-3.1-pro-preview": {
        ...
        "rpd": 25,
        "tpm": 64000,
        "desc": "Новейший, лучший reasoning 🏆",
        "provider": "Google",
    },
    "gemini-3-flash-preview": {
        ...
        "rpd": 100,
        "tpm": 250000,
        "desc": "Gemini 3 скорость + умность",
        "provider": "Google",
    },
    # ===== GOOGLE GEMINI 2.5 =====
    "gemini-2.5-flash": {
    ...
    "meta-llama/llama-4-maverick-17b-128e-instruct": {
        ...
        "call": "meta-llama/llama-4-maverick-17b-128e-instruct",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 131072,
        "desc": "Llama 4",
    ...
    "meta-llama/llama-4-scout-17b-16e-instruct": {
        "label": "Llama 4 Scout",
        "call": "meta-llama/llama-4-scout-17b-16e-instruct",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 131072,
        "desc": "10M контекст",
    ...
    "qwen-qwq-32b": {
        ...
        "rpd": 1000,
        "tpm": 131072,
        "desc": "Reasoning Alibaba",
        "provider": "Groq",
    },
    "moonshotai/kimi-k2-instruct-0905": {
        "label": "Kimi K2 🆕",
        "call": "moonshotai/kimi-k2-instruct-0905",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 262144,
        "desc": "262K контекст",
    ...
    "groq/compound": {
        "label": "Compound 🌐 (поиск)",
        "call": "groq/compound",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 131072,
        "desc": "Реальный поиск в интернете 🔍",
        "provider": "Groq",
    },
    "groq/compound-mini": {
        "label": "Compound Mini 🌐 (поиск)",
        "call": "groq/compound-mini",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 131072,
        "desc": "Поиск в интернете, быстрее",
        "provider": "Groq",
    },
    ...
    "cerebras/llama-3.3-70b": {
        ...
        "rpd": 14400,
        "tpm": 8192,
        "desc": "2000+ tok/s — молниеносный 💨",
        "provider": "Cerebras",
    },
    "cerebras/llama-4-scout": {
        "label": "Cerebras Llama 4 Scout",
        "call": "cerebras/llama-4-scout",
        "rpm": 30,
        "rpd": 14400,
        "tpm": 8192,
        "desc": "Llama 4 быстро через Cerebras",
        "provider": "Cerebras",
    },
    "cerebras/qwen-3-235b": {
        ...
        "rpd": 14400,
        "tpm": 8192,
        "desc": "Мощный Qwen3, быстро",
        "provider": "Cerebras",
    },
    # ===== MISTRAL (бесплатно: 1B токен/месяц) =====
    "mistral/mistral-small-latest": {
        "label": "Mistral Small 🌊",
        "call": "mistral/mistral-small-latest",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 128000,
        "desc": "Mistral Small — 1B tok/month бесплатно",
        "provider": "Mistral",
    },
    "mistral/mistral-medium-latest": {
        "label": "Mistral Medium 🌊",
        "call": "mistral/mistral-medium-latest",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 128000,
        "desc": "Mistral Medium",
    ...
    "mistral/devstral-small": {
        "label": "Devstral (код) 💻",
        "call": "mistral/devstral-small",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 128000,
        "desc": "Лучший для кода (Mistral, бесплатно)",
        "provider": "Mistral",
    },
    "mistral/mistral-nemo": {
        "label": "Mistral Nemo 12B",
        "call": "mistral/mistral-nemo",
        "rpm": 30,
        "rpd": 1000,
        "tpm": 128000,
        "desc": "Mistral Nemo 12B",
        "provider": "Mistral",
    },
}
...
class AternosManager:
    ...
    async def _login(self, user, pw):
        ...
        try:
            ...
            m = _re.search(r'AJAX_TOKEN[^"]*"([^"]+)"', html) or _re.search(
                r"AJAX_TOKEN[^']*'([^']+)'", html
            )
            if m:
    ...
    async def _find_server_id(self):
        ...
        try:
            ...
            # Ищем data-id в HTML
            ids = _re.findall('data-id="([^"]+)"', html) or _re.findall(
                "data-id='([^']+)'", html
            )
            if ids:
...
class MCBotManager:
    ...
    def _notify(self, text: str, delete_after: int = 20):
        ...
        if loop and not loop.is_closed():
            ...
            async def _send():
                ...
                if ch:
                    try:
                        await ch.send(text, delete_after=delete_after)
                    except:
                        pass
            ...
            if line.startswith("{"):
                try:
                    ...
                    if t == "spawned":
                        ...
                        self._notify(
                            f"✅ **{self.username}** зашёл на `{self.host}` | Позиция: {pos_str}"
                        )
                        ...
                        if self.health <= 6:
                            self._notify(
                                f"⚠️ Низкое HP: ❤️ `{self.health}/20` 🍖 `{self.food}/20`"
                            )
                        ...
                        self._disconnect_time = time.time()
                        self._notify(
                            "🔌 **Соединение потеряно.** Реконнект через 5с..."
                        )
                        ...
                        self._notify(
                            f"📤 **Отправлено в MC:** `{m.get('text', '')[:200]}`"
                        )

                    elif t == "reconnect_stopped":
                        self._notify(
                            "🛑 **Реконнект остановлен.** Нажми 🔄 Реконнект чтобы снова подключиться.",
                            delete_after=60,
                        )

                    elif t == "reconnect_enabled":
                        self._notify(
                            "🔄 **Реконнект запущен...** Подключаюсь...",
                            delete_after=20,
                        )
                ...
                is_epipe = any(
                    x in line for x in ["EPIPE", "ECONNRESET", "write EPIPE"]
                )
                important = not is_epipe and any(
                    x in line.lower()
                    for x in [
                        "error",
                        "ошибка",
    ...
    def stop(self):
        self.send("!стоп")
        try:
            self._proc and self._proc.wait(timeout=3)
        except:
...
async def generate_image(prompt, style="auto"):
    ...
    headers = (
        {"Authorization": f"Bearer {POLLINATIONS_KEY}"} if POLLINATIONS_KEY else {}
    )
...
async def generate_video(prompt: str) -> bytes:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.post(
            "https://gen.pollinations.ai/v1/video", headers=headers, json=payload
        ) as resp:
                            ...
                            async with session.get(ep, headers=headers) as sr:
                                if sr.status == 200:
                                    ...
                                    if sdata.get("status") in (
                                        ...
                                        raise ValueError(
                                            f"Генерация провалилась: {sdata.get('error', '')}"
                                        )
                        except ValueError:
                            raise
                        except Exception:
...
async def _call_model(model_name, prompt, history, media_parts=None):
    ...
    if is_cerebras:
        ...
        resp = cerebras_client.chat.completions.create(
            model=real, messages=msgs, max_tokens=4096, temperature=0.7
        )
    ...
    if is_hf:
        ...
        for h in history[-10:]:
            ...
            msgs.append({"role": role, "content": h.get("parts", [""])[0]})
        msgs.append({"role": "user", "content": prompt})
        resp = hf_cli.chat.completions.create(
            model=real, messages=msgs, max_tokens=4096, temperature=0.7
        )
        return resp.choices[0].message.content.strip()
    ...
    if is_groq:
                    ...
                    user_content.append(
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:{mp['mime_type']};base64,{b64}"
                            },
                        }
                    )
        ...
        resp = groq_client.chat.completions.create(
            model=model_name, messages=msgs, max_tokens=4096, temperature=0.8
        )
        answer = resp.choices[0].message.content
        used_tools = getattr(resp.choices[0].message, "executed_tools", None) or []
        did_search = any(
            getattr(t, "type", "")
            in ("web_search", "browser_automation", "visit_website")
            for t in used_tools
        )
        if did_search:
    ...
    for h in hist_use:
        ...
        contents.append(
            genai_types.Content(role=role, parts=[genai_types.Part(text=parts_text)])
        )
    ...
    if media_parts:
        user_parts = [
            genai_types.Part(
                text=prompt if not is_gemma else SYSTEM_PROMPT + "\n\nЗапрос: " + prompt
            )
        ]
        for mp in media_parts:
            user_parts.append(
                genai_types.Part(
                    inline_data=genai_types.Blob(
                        mime_type=mp["mime_type"], data=mp["data"]
                    )
                )
            )
        ...
        contents.append(
            genai_types.Content(role="user", parts=[genai_types.Part(text=actual)])
        )
        ...
        if groq_client:
            resp = groq_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Extract a short 2-6 word search query from the user message. Reply ONLY with the query, no quotes.",
                    },
...
async def ask_ai(uid, prompt, channel=None, media_parts=None):
    ...
    if owner_model:
        ...
        if (
            _wants_web_search(prompt)
            and cur not in WEB_SEARCH_MODELS
            and not search_context
        ):
            order = ["groq/compound", "groq/compound-mini"] + [
                m for m in AUTO_FALLBACK_ORDER if m not in WEB_SEARCH_MODELS
            ]
        elif _needs_uncensored(prompt) and cur not in {
            "qwen/qwen3-32b",
            "qwen-qwq-32b",
        }:
            first = [
    ...
    for model_name in order:
        try:
            answer_text = await _call_model(
                model_name, augmented_prompt, user_hist, media_parts
            )
            if _is_refusal(answer_text) and model_name not in WEB_SEARCH_MODELS:
                last_err = f"{model_name} отказал"
                continue
            ...
            if "413" in err_str or "too large" in err_str or "too long" in err_str:
                ...
                last_err = f"Запрос слишком большой ({str(e)[:80]})"
                continue
...
async def send_ai_reply(
    interaction, answer_text, ephemeral=True, model_name="", used_ddg=False
):
    ...
    if code:
        if len(code) < 1500:
            ...
            await interaction.followup.send(
                content=prefix + msg,
                file=discord.File(
                    fp=io.BytesIO(code.encode("utf-8")), filename=f"{label_f}.{ext}"
                ),
                ephemeral=ephemeral,
            )
    else:
...
class AskAIModal(discord.ui.Modal, title="Nexus AI — Задать вопрос"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if bal < TOKEN_COST_AI:
            ...
            return
        success, answer_text, used_model, used_ddg = await ask_ai(
            uid, self.prompt.value
        )
        if not success:
...
class UniversalAITextModal(discord.ui.Modal, title="Nexus AI — Универсальный запрос"):
    prompt = discord.ui.TextInput(
        label="Вопрос / задача",
        style=discord.TextStyle.paragraph,
        placeholder="Спроси что угодно, или !img <описание> для картинки",
        required=True,
        max_length=2000,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if text.lower().startswith(("!img ", "!имг ", "сгенерируй ", "нарисуй ")):
            ...
            if bal < TOKEN_COST_IMG:
                await interaction.followup.send(
                    f"❌ Not enough tokens! Have **{bal}**, need **{TOKEN_COST_IMG}**.",
                    ephemeral=True,
                )
                return
            img_prompt = re.sub(
                r"^(!img |!имг |сгенерируй |нарисуй )", "", text, flags=re.IGNORECASE
            ).strip()
            try:
                ...
                await interaction.followup.send(
                    ...
                    file=discord.File(
                        fp=io.BytesIO(img_bytes), filename="nexus_ai.png"
                    ),
                    ephemeral=True,
                )
            except Exception as e:
        ...
        if bal < TOKEN_COST_AI:
            await interaction.followup.send(
                f"❌ Not enough tokens! Have **{bal}**, need **{TOKEN_COST_AI}**.",
                ephemeral=True,
            )
            return
...
class UniversalAIView(discord.ui.View):
    ...
    async def file_btn(self, interaction, button):
        embed = discord.Embed(
            title="📎 Как прикрепить файл",
            description="Отправь в канал:\n```?ai <вопрос>```\nИ прикрепи файл/фото.",
            color=0x3498DB,
        )
...
class WebSearchModal(discord.ui.Modal, title="🌐 Поиск в интернете"):
    ...
    async def on_submit(self, interaction):
        ...
        bal = get_tokens(uid)
        if bal < TOKEN_COST_AI:
            await interaction.followup.send(f"❌ Not enough tokens!", ephemeral=True)
            return
        try:
            answer_text = await _call_model(
                "groq/compound", self.query.value, get_user_history(uid)
            )
        except Exception:
            success, answer_text, _um, used_ddg = await ask_ai(uid, self.query.value)
            if not success:
                await interaction.followup.send(f"❌ {answer_text}", ephemeral=True)
                return
        spend_tokens(uid, TOKEN_COST_AI)
        await send_ai_reply(interaction, answer_text)
...
class AIPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Universal AI",
        style=discord.ButtonStyle.primary,
        custom_id="panel_universal",
        emoji="🌟",
        row=0,
    )
    async def universal_btn(
        ...
        await interaction.response.send_message(
            embed=embed, view=UniversalAIView(), ephemeral=True
        )

    @discord.ui.button(
        label="History",
        style=discord.ButtonStyle.secondary,
        custom_id="panel_lastmsg",
        emoji="📜",
        row=0,
    )
    async def lastmsg_btn(self, interaction, button):
        ...
        while i >= 0 and len(pairs) < 7:
            if hist[i]["role"] == "model" and i > 0 and hist[i - 1]["role"] == "user":
                q = hist[i - 1].get("parts", [""])[0]
                a = hist[i].get("parts", [""])[0]
                pairs.append((q, a))
                i -= 2
            else:
                i -= 1
    ...
    @discord.ui.button(
        label="Set Model",
        style=discord.ButtonStyle.primary,
        custom_id="panel_setmodel",
        emoji="⚙️",
        row=1,
    )
    async def setmodel_btn(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        ...
        owner_model = get_owner_model(uid)
        owner_label = (
            OWNER_EXCLUSIVE_MODELS.get(owner_model, {}).get("label", owner_model)
            if owner_model
            else "Auto (global)"
        )
        embed = discord.Embed(
        ...
        await interaction.response.send_message(
            embed=embed, view=ModelSelectView(is_owner=True), ephemeral=True
        )

    @discord.ui.button(
        label="Models",
        style=discord.ButtonStyle.secondary,
        custom_id="panel_model",
        emoji="🤖",
        row=1,
    )
    async def model_btn(
    ...
    async def limit_btn(self, interaction, button):
        ...
        embed = discord.Embed(
            title="📊 Limits",
            description=f"**{m['label']}**\nRPM: {m['rpm']} | RPD: {m['rpd']} | TPM: {m['tpm']:,}"
            if m
            else f"`{get_current_model()}`",
            color=0x9B59B6,
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
class ModelSelectGoogle(discord.ui.Select):
    def __init__(self):
        opts = [
            discord.SelectOption(
                ...
                emoji="🏆",
                description="Новейший • 25 req/day",
            ),
            discord.SelectOption(
                label="Gemini 3 Flash Preview 🆕",
                value="gemini-3-flash-preview",
                emoji="🌟",
                description="Gemini 3 быстрый • 100 req/day",
            ),
            discord.SelectOption(
                label="Gemini 2.5 Flash ⭐",
                value="gemini-2.5-flash",
                emoji="🔥",
                description="Рекомендуется • 500 req/day",
            ),
            discord.SelectOption(
                label="Gemini 2.5 Flash-Lite",
                value="gemini-2.5-flash-lite",
                emoji="💨",
                description="Самая быстрая • 1500 req/day",
            ),
            discord.SelectOption(
                label="Gemini 2.5 Pro",
                value="gemini-2.5-pro",
                emoji="👑",
                description="Умная 2.5 • 100 req/day",
            ),
            discord.SelectOption(
                label="Gemini 2.0 Flash",
                value="gemini-2.0-flash",
                emoji="⚡",
                description="1M контекст • 1500 req/day",
            ),
            discord.SelectOption(
                label="Gemma 3 27B",
                value="gemma-3-27b-it",
                emoji="🧬",
                description="Open-source • 50 req/day",
            ),
            discord.SelectOption(
                label="Gemma 3 12B",
                value="gemma-3-12b-it",
                emoji="🔬",
                description="Баланс • 100 req/day",
            ),
            discord.SelectOption(
                label="Gemma 3 4B",
                value="gemma-3-4b-it",
                emoji="📱",
                description="Лёгкая • 300 req/day",
            ),
        ]
        super().__init__(
            placeholder="🌐 Google модели...",
            min_values=1,
            max_values=1,
            options=opts,
            custom_id="select_google",
            row=0,
        )
    ...
    async def callback(self, interaction):
        ...
        await interaction.response.edit_message(
            content=f"✅ **{self.values[0]}**", embed=None, view=None
        )
...
class ModelSelectGroq(discord.ui.Select):
    def __init__(self):
        opts = [
            discord.SelectOption(
                label="🔍 Compound (поиск)", value="groq/compound", emoji="🌐"
            ),
            discord.SelectOption(
                label="🔎 Compound Mini", value="groq/compound-mini", emoji="🌐"
            ),
            discord.SelectOption(
                label="Llama 3.3 70B ⭐", value="llama-3.3-70b-versatile", emoji="🦙"
            ),
            discord.SelectOption(
                label="Llama 3.1 8B Instant", value="llama-3.1-8b-instant", emoji="💨"
            ),
            discord.SelectOption(
                label="Llama 4 Maverick 17B",
                value="meta-llama/llama-4-maverick-17b-128e-instruct",
                emoji="🦙",
                description="Новейший Llama 4",
            ),
            discord.SelectOption(
                label="GPT-OSS 120B", value="openai/gpt-oss-120b", emoji="🤖"
            ),
            discord.SelectOption(
                label="GPT-OSS 20B", value="openai/gpt-oss-20b", emoji="⚡"
            ),
            discord.SelectOption(
                label="Qwen3 32B 🧠", value="qwen/qwen3-32b", emoji="🧠"
            ),
            discord.SelectOption(
                label="Qwen QwQ 32B", value="qwen-qwq-32b", emoji="🌟"
            ),
            discord.SelectOption(
                label="Kimi K2 🆕", value="moonshotai/kimi-k2-instruct-0905", emoji="🌙"
            ),
        ]
        super().__init__(
            placeholder="⚡ Groq модели...",
            min_values=1,
            max_values=1,
            options=opts,
            custom_id="select_groq",
            row=1,
        )
    ...
    async def callback(self, interaction):
        ...
        set_current_model(self.values[0])
        await interaction.response.edit_message(
            content=f"✅ **{self.values[0]}** [Groq]", embed=None, view=None
        )
...
class AutoToggleButton(discord.ui.Button):
    ...
    async def callback(self, interaction):
        ...
        await interaction.followup.send(
            f"🔄 Авто-режим {'ВКЛ ✅' if new_state else 'ВЫКЛ ❌'}", ephemeral=True
        )
    ...
    def __init__(self):
        opts = [
            discord.SelectOption(
                ...
                emoji="⚡",
                description="2000+ tok/s • 1M tok/day бесплатно",
            ),
            discord.SelectOption(
                label="🧠 Cerebras Qwen3 235B",
                value="cerebras/qwen-3-235b",
                emoji="🧠",
                description="Мощный • быстрый",
            ),
            discord.SelectOption(
                label="🚀 Cerebras Llama 4 Scout",
                value="cerebras/llama-4-scout",
                emoji="🚀",
                description="131K контекст",
            ),
            discord.SelectOption(
                label="🌊 Mistral Small",
                value="mistral/mistral-small-latest",
                emoji="🌊",
                description="1B tok/month бесплатно",
            ),
            discord.SelectOption(
                label="💻 Mistral Devstral (код)",
                value="mistral/devstral-small",
                emoji="💻",
                description="Лучший coding • бесплатно",
            ),
            discord.SelectOption(
                label="🔵 Mistral Nemo 12B",
                value="mistral/mistral-nemo",
                emoji="🔵",
                description="Лёгкий, быстрый",
            ),
        ]
        super().__init__(
            placeholder="🆓 Cerebras / Mistral (бесплатно)...",
            min_values=1,
            max_values=1,
            options=opts,
            custom_id="select_extra",
            row=2,
        )
...
class ModelSelectHF(discord.ui.Select):
    def __init__(self):
        opts = [
            ...
            discord.SelectOption(
                label="🦙 Llama 3.3 70B (HF)", value="hf/llama-3.3-70b"
            ),
            discord.SelectOption(label="🔷 Qwen2.5 72B (HF)", value="hf/qwen2.5-72b"),
            discord.SelectOption(
                label="💨 Mistral Small 3.1 (HF)", value="hf/mistral-small-3.1"
            ),
            discord.SelectOption(label="🔄 Сбросить на Auto", value="__reset__"),
        ]
        super().__init__(
            placeholder="🎯 Моя модель (Owner HF)...",
            min_values=1,
            max_values=1,
            options=opts,
            custom_id="select_hf_owner",
            row=3,
        )
    ...
    async def callback(self, interaction):
        ...
        if val == "__reset__":
            ...
            await interaction.response.edit_message(
                content="✅ **Reset to Auto**", embed=None, view=None
            )
        else:
            ...
            await interaction.response.edit_message(
                content=f"✅ **{label}** {hf_ok}", embed=None, view=None
            )
...
class HistoryView(View):
    ...
    async def show_history(self, interaction, button):
        ...
        if not vh:
            return await interaction.response.send_message(
                "History is empty.", ephemeral=True
            )
...
class RoleView(View):
    ...
    async def toggle_role(self, interaction, role_id):
        ...
        if role in interaction.user.roles:
            ...
            await interaction.response.send_message(
                f"❌ **{role.name}** убрана.", ephemeral=True
            )
        else:
            await interaction.user.add_roles(role)
            await interaction.response.send_message(
                f"✅ **{role.name}** выдана!", ephemeral=True
            )

    @discord.ui.button(
        label="Executer UPD", style=discord.ButtonStyle.primary, custom_id="role_exec"
    )
    async def exec_btn(self, interaction, button):
        await self.toggle_role(interaction, ROLE_EXECUTER_ID)

    @discord.ui.button(
        label="Roblox UPD", style=discord.ButtonStyle.success, custom_id="role_roblox"
    )
    async def roblox_btn(self, interaction, button):
        await self.toggle_role(interaction, ROLE_ROBLOX_ID)

    @discord.ui.button(
        label="Script UPD", style=discord.ButtonStyle.danger, custom_id="role_script"
    )
    async def script_btn(self, interaction, button):
...
class AternosStartModal(discord.ui.Modal, title="🚀 Запустить Aternos сервер"):
    aternos_user = discord.ui.TextInput(
        label="Логин Aternos", placeholder="твой_логин", required=True, max_length=64
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.defer(ephemeral=True, thinking=True)
        await interaction.followup.send(
            "⏳ Входжу в Aternos и запускаю сервер...", ephemeral=True
        )
        ...
        if addr and "✅" in res:
            ...
            if not ok:
                await interaction.followup.send("❌ Node.js не найден!", ephemeral=True)
                return
            for _ in range(25):
                await asyncio.sleep(2)
                if mc_bot.connected:
                    await interaction.followup.send(
                        f"✅ Бот зашёл на `{host}:{port}`!", ephemeral=True
                    )
                    return
            await interaction.followup.send(
                "⏳ Бот запущен, жду спавн...", ephemeral=True
            )
...
class MCJoinModal(discord.ui.Modal, title="🔌 Подключить MC бота"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if ver.lower() in ("auto", "авто", "false", ""):
            ver = "auto"
        ...
        for _ in range(25):
            ...
            if mc_bot.connected:
                ...
                return
        await interaction.followup.send(
            f"⏳ Бот запущен, но спавн ещё не случился. Подожди немного.",
            ephemeral=True,
        )
...
class MCChatModal(discord.ui.Modal, title="💬 Написать в MC / Команда"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not mc_bot.connected:
            await interaction.followup.send("❌ Бот не на сервере.", ephemeral=True)
            return
        mc_bot.send(line)
        await interaction.followup.send(
            f"✅ Отправлено: `{line[:100]}`", ephemeral=True
        )
...
class MCPanelView(discord.ui.View):
    ...
    async def btn_start(
        ...
        if ATERNOS_USER and ATERNOS_PASS:
            ...
            if "✅" in res and MC_SERVER:
                ...
                if not ok:
                    await interaction.followup.send(
                        "❌ Node.js не найден.", ephemeral=True
                    )
                    return
                for _ in range(20):
                    await asyncio.sleep(2)
                    if mc_bot.connected:
                        await interaction.followup.send(
                            f"✅ Зашёл как **{MC_USERNAME}**!", ephemeral=True
                        )
                        return
                await interaction.followup.send(
                    "⏳ Бот запущен, жду спавн...", ephemeral=True
    ...
    async def btn_status(
        ...
        beh_emoji = {"мирный": "🕊️", "защита": "🛡️", "агрессия": "⚔️"}.get(
            mc_bot.behavior, "❓"
        )
        embed = discord.Embed(
            title="📊 Minecraft Status",
            color=0x2ECC71 if mc_bot.connected else 0xE74C3C,
        )
        embed.add_field(
            name="Aternos", value=f"{ic.get(st_a, '⚪')} `{st_a}`", inline=True
        )
        embed.add_field(
            name="MC Бот",
            value=f"{'✅ Online' if mc_bot.connected else '❌ Offline'}",
            inline=True,
        )
        embed.add_field(
            name="❤️ HP / 🍖",
            value=f"`{mc_bot.health}/20` / `{mc_bot.food}/20`",
            inline=True,
        )
        embed.add_field(
            name="📍 Позиция",
            value=f"`{mc_bot.pos}`" if mc_bot.pos else "*неизвестно*",
            inline=False,
        )
        features_line = (
            ("🟢" if mc_bot.afk else "🔴")
            + " АФК  "
            + ("🟢" if mc_bot.anti_afk else "🔴")
            + " Анти-АФК  "
            + ("🟢" if mc_bot.auto_eat else "🔴")
            + " Автоеда  "
            + ("🟢" if mc_bot.auto_armor else "🔴")
            + " Автоброня\n"
            + beh_emoji
            + " Режим: **"
            + mc_bot.behavior
    ...
    @discord.ui.button(
        label="💤 АФК",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_afk_btn",
        row=1,
    )
    async def btn_afk(self, interaction, button):
        if not self._is_owner(interaction):
            return await self._deny(interaction)
        if self._need_bot(interaction):
            return await interaction.response.send_message(
                "❌ Бот не на сервере.", ephemeral=True
            )
        mc_bot.afk = not mc_bot.afk
        mc_bot.send("!афк")
        await interaction.response.send_message(
            f"{'💤 АФК включён' if mc_bot.afk else '✅ АФК выключен'}", ephemeral=True
        )

    @discord.ui.button(
        label="🤖 Анти-АФК",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_antiafk_btn",
        row=1,
    )
    async def btn_anti_afk(self, interaction, button):
        ...
        mc_bot.send("!антиафк")
        await interaction.response.send_message(
            f"{'🟢 Анти-АФК вкл' if mc_bot.anti_afk else '🔴 Анти-АФК выкл'}",
            ephemeral=True,
        )
    ...
    async def btn_autoeat(self, interaction, button):
        ...
        mc_bot.send("!автоеда")
        await interaction.response.send_message(
            f"{'🟢 Автоеда вкл' if mc_bot.auto_eat else '🔴 Автоеда выкл'}",
            ephemeral=True,
        )
    ...
    async def btn_autoarmor(self, interaction, button):
        ...
        await interaction.response.send_message(
            f"{'🟢 Автоброня вкл' if mc_bot.auto_armor else '🔴 Автоброня выкл'}",
            ephemeral=True,
        )
    ...
    async def btn_behavior(self, interaction, button):
        ...
        await interaction.response.send_message(
            f"{emoji} Режим: **{nxt}**", ephemeral=True
        )
    ...
    @discord.ui.button(
        label="🚫 Стоп реконнект",
        style=discord.ButtonStyle.danger,
        custom_id="mc_stopreconnect_btn",
        row=2,
    )
    ...
    # ══ ROW 2 — Движение ═════════════════════════════════════════════════════
    @discord.ui.button(
        label="⬆️", style=discord.ButtonStyle.secondary, custom_id="mc_fwd", row=3
    )
    async def btn_fwd(self, interaction, button):
        ...
        if self._need_bot(interaction):
            return await interaction.response.send_message(
                "❌ Не на сервере.", ephemeral=True
            )
        mc_bot.send("!run")
        await interaction.response.send_message("⬆️ Вперёд!", ephemeral=True)

    @discord.ui.button(
        label="⬇️", style=discord.ButtonStyle.secondary, custom_id="mc_back", row=3
    )
    async def btn_back(self, interaction, button):
        ...
        mc_bot.send("!back")
        await interaction.response.send_message("⬇️ Назад!", ephemeral=True)

    @discord.ui.button(
        label="🐰 Прыжок",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_jump_btn",
        row=3,
    )
    async def btn_jump(self, interaction, button):
        ...
        mc_bot.send("!прыгни")
        await interaction.response.send_message("🐰 Прыгаю!", ephemeral=True)

    @discord.ui.button(
        label="🌀 Spin",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_spin_btn",
        row=3,
    )
    async def btn_spin(self, interaction, button):
        ...
        mc_bot.send("!кружись")
        await interaction.response.send_message("🌀 Кружусь!", ephemeral=True)

    @discord.ui.button(
        label="🛑 Стоп",
        style=discord.ButtonStyle.danger,
        custom_id="mc_stop_move_btn",
        row=3,
    )
    async def btn_stop_move(self, interaction, button):
        ...
        mc_bot.send("!стоп")
        await interaction.response.send_message("🛑 Стоп!", ephemeral=True)

    # ══ ROW 3 — Действия ═════════════════════════════════════════════════════
    @discord.ui.button(
        label="😴 Спать",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_sleep_btn",
        row=4,
    )
    async def btn_sleep(self, interaction, button):
        ...
        mc_bot.send("!спать")
        await interaction.response.send_message("😴 Иду спать...", ephemeral=True)

    @discord.ui.button(
        label="🍖 Поесть",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_eat_btn",
        row=4,
    )
    async def btn_eat(self, interaction, button):
        ...
        mc_bot.send("!поешь")
        await interaction.response.send_message("😋 Ем!", ephemeral=True)

    @discord.ui.button(
        label="🎒 Инвентарь",
        style=discord.ButtonStyle.secondary,
        custom_id="mc_inv_btn",
        row=4,
    )
    async def btn_inv(self, interaction, button):
        ...
        mc_bot.send("!инвентарь")
        await interaction.response.send_message(
            "🎒 Запрошен инвентарь — смотри в логах.", ephemeral=True
        )

    @discord.ui.button(
        label="❤️ HP", style=discord.ButtonStyle.secondary, custom_id="mc_hp_btn", row=4
    )
    ...
    async def btn_pos(self, interaction, button):
        ...
        mc_bot.send("!где")
        await interaction.response.send_message(
            f"📍 `{mc_bot.pos}`" if mc_bot.pos else "📍 Позиция неизвестна",
            ephemeral=True,
        )
...
async def ensure_mc_panel(channel):
    ...
    embed = discord.Embed(
        title="🎮 Minecraft Control Panel", description=desc, color=0x00B4D8
    )
...
async def on_message(message):
    ...
    if message.channel.id == MC_CHANNEL_ID:
        ...
        if cmd == "mcstart":
            ...
            if "✅" in res and addr:
                ...
                try:
                    port = int(raw.split(":")[1]) if ":" in raw else MC_PORT_NUM
                except:
                    port = MC_PORT_NUM
                if mc_bot.connected:
                    mc_bot.stop()
                ...
                for _ in range(15):
                    await asyncio.sleep(2)
                    if mc_bot.connected:
                        await m2.edit(content=f"✅ Зашёл как **{MC_USERNAME}**!")
                        return
        ...
        #   !join play.server.com:25565 1.21.8 MyBot
            ...
            m = await message.channel.send(
                f"🔌 Подключаюсь к `{host}:{port}` v`{ver}` как **{user}**..."
            )
            ok = mc_bot.start(host, port, user, ver)
            if not ok:
                await m.edit(content="❌ Node.js не найден!")
                return
            for _ in range(15):
                await asyncio.sleep(2)
                if mc_bot.connected:
                    await m.edit(content=f"✅ Зашёл на `{host}:{port}` как **{user}**!")
                    return
        ...
        # !leave — выйти
            ...
            mc_bot.stop()
            await message.channel.send("👋 Вышел с сервера.")
            return
        ...
        if cmd == "say":
            if not _chk():
                await message.channel.send("❌ Бот не на сервере.")
                return
        ...
        if cmd in PASSTHROUGH_CMDS or content.startswith("!"):
            ...
            await message.channel.send(
                f"✅ Команда отправлена: `{content[:120]}`", delete_after=5
            )
            return
    ...
    if message.channel.id == AI_CHANNEL_ID:
        ...
        if content.startswith(("?ai ", "?аи ")):
            ...
            if bal < TOKEN_COST_AI:
                try:
                    ...
                    pass
                await message.channel.send(
                    f"❌ {message.author.mention} Not enough tokens! Have **{bal}**, need **{TOKEN_COST_AI}**.",
                    delete_after=15,
                )
                return
            ...
            async with message.channel.typing():
                success, answer_text, used_model_chat, used_ddg_chat = await ask_ai(
                    message.author.id, full_prompt, media_parts=media_parts
                )
            ...
            if not success:
                await message.channel.send(
                    f"❌ {message.author.mention} Ошибка: {answer_text}",
                    delete_after=20,
                )
                return
        ...
        if content.startswith(("?img ", "?имг ")):
            ...
            if bal < TOKEN_COST_IMG:
                try:
                    ...
                    pass
                await message.channel.send(
                    f"❌ {message.author.mention} Not enough tokens! Have **{bal}**, need **{TOKEN_COST_IMG}**.",
                    delete_after=15,
                )
                return
            try:
        ...
        if content.startswith(("?video ", "?vid ", "?видео ")):
            video_prompt = (
                message.content.split(None, 1)[1].strip()
            ...
            status_msg = await message.channel.send(
                f"🎬 {message.author.mention} Генерирую видео... (может занять 1-3 мин)",
                delete_after=300,
            )
            async with message.channel.typing():
                try:
                    ...
                    await message.channel.send(
                        ...
                        file=discord.File(
                            fp=io.BytesIO(video_bytes), filename="nexus_video.mp4"
                        ),
                        delete_after=300,
                    )
                except Exception as e:
                    try:
                        ...
                        pass
                    await message.channel.send(
                        f"❌ {message.author.mention} Video error: {e}", delete_after=20
                    )
            return
        ...
        if content.startswith(("!mymodel", "!myai", "!моямодель")):
            ...
            if not arg:
                ...
                cur_label = (
                    OWNER_EXCLUSIVE_MODELS.get(cur, {}).get("label", cur)
                    if cur
                    else "Auto (global)"
                )
                await message.channel.send(
        ...
        if content.startswith(("!tokens", "!token balance", "!баланс")):
            ...
            embed = discord.Embed(
                title="💎 Token Balance",
                description=f"**{message.author.display_name}** — `{bal}` tokens\n\n{bar}\n\n📝 AI: **{TOKEN_COST_AI}** | 🎨 Image: **{TOKEN_COST_IMG}** | 🎬 Video: **{TOKEN_COST_VIDEO}**\n*Monthly: +{TOKEN_MONTHLY}*",
                color=0x00FBFF,
            )
        ...
        if content.startswith(("?clear", "?клир")):
            ...
            await message.channel.send(
                f"✅ {message.author.mention} История очищена.", delete_after=8
            )
            return
...
async def token_cmd(ctx, *, args: str = None):
    try:
        ...
        pass
    if not any(r.id == OWNER_ROLE_ID for r in ctx.author.roles):
        return
    if not args:
        await ctx.send("Usage: `!token <@mention|username> <amount>`", delete_after=12)
        return
    parts = args.strip().rsplit(None, 1)
    if len(parts) < 2:
        await ctx.send("Specify amount!", delete_after=10)
        return
    user_query, amount_str = parts[0].strip(), parts[1].strip()
    try:
        amount = int(amount_str)
    except:
        await ctx.send(f"`{amount_str}` is not a number.", delete_after=10)
        return
    member = None
    try:
    ...
    if not member:
        ...
        for m in ctx.guild.members:
            if q in m.name.lower() or q in m.display_name.lower():
                ...
                break
    if not member:
        await ctx.send(f"User `{user_query}` not found.", delete_after=12)
        return
    add_tokens(member.id, amount)
    new_bal = get_tokens(member.id)
    embed = discord.Embed(
        title="Tokens Added",
        description=f"**{member.display_name}** +**{amount}** → **{new_bal}** tokens",
        color=0x2ECC71,
    )
...
async def check_exploits():
    ...
    if not exploit_msg_id[0]:
        async for msg in channel.history(limit=10):
            if (
                msg.author == bot.user
                and msg.embeds
                and "🛡️ Nexus Exploit Status" in str(msg.embeds[0].title)
            ):
    ...
    if not edited:
        try:
            ...
            print(f"[Exploit update] send error: {e}")
...
async def update_roblox_msg(channel, live, future, is_update=False):
    ...
    if not edited:
        try:
            ...
            print(f"[Roblox update] send error: {e}")
...
async def monthly_token_refill():
    ...
    for doc in col_tokens.find():
        col_tokens.update_one(
            {"_id": doc["_id"]},
            {"$set": {"tokens": doc.get("tokens", 0) + TOKEN_MONTHLY}},
        )
        count += 1
