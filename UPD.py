...
const bot = mineflayer.createBot({
    ...
    version: process.env.MC_VERSION||'1.20.1', auth:'offline'
})
...
class MCBotManager:
    ...
    def start(self, host, port, user, ver):
        import pathlib
        js = pathlib.Path("_mc.js")
        js.write_text(_MC_JS, encoding="utf-8")
        env = {**os.environ, "MC_HOST": host, "MC_PORT": str(port),
    ...
    def _read(self):
        for line in self._proc.stdout:
            try:
                ...
                elif t in ("kicked","end"): self.connected = False
            except: pass

    def cmd(self, c, **kw):
        if not self._proc or self._proc.poll() is not None: return False
        try:
            self._proc.stdin.write(json.dumps({"cmd": c, **kw}) + "\n")
...
class MCPanelView(discord.ui.View):
    ...
    async def btn_start(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not ATERNOS_USER or not ATERNOS_PASS:
            await interaction.followup.send("❌ `ATERNOS_USER` / `ATERNOS_PASS` не заданы в env vars.", ephemeral=True); return
        await interaction.followup.send("⏳ Запускаю Aternos...", ephemeral=True)
        ...
        if "✅" in res and MC_SERVER:
            ...
            if not ok:
                await interaction.followup.send("❌ Node.js не найден. Установи: `npm install mineflayer`", ephemeral=True); return
            for _ in range(20):
                await asyncio.sleep(2)
                if mc_bot.connected:
                    await interaction.followup.send(f"✅ MC: зашёл как **{MC_USERNAME}**!", ephemeral=True); return
            await interaction.followup.send("⏳ MC бот запущен, ждёт спавн...", ephemeral=True)

    @discord.ui.button(label="⏹ Остановить", style=discord.ButtonStyle.danger, custom_id="mc_stop", emoji="🛑", row=0)
    ...
    async def btn_status(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="MC Bot",    value=f"{'✅ Online' if mc_bot.connected else '❌ Offline'}",                      inline=True)
        embed.add_field(name="Health",    value=f"❤️ `{mc_bot.health}/20` 🍖 `{mc_bot.food}/20`",                           inline=False)
        embed.add_field(name="Position",  value=f"📍 `{mc_bot.pos}`" if mc_bot.pos else "📍 *неизвестно*",                  inline=False)
    ...
    async def btn_join(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not MC_SERVER:
            await interaction.response.send_message("❌ `MC_SERVER` не задан в env vars.", ephemeral=True); return
        await interaction.response.defer(ephemeral=True)
        ok = mc_bot.start(MC_SERVER, MC_PORT_NUM, MC_USERNAME, MC_VERSION)
        if not ok:
        ...
        for _ in range(20):
            await asyncio.sleep(2)
            if mc_bot.connected:
                await interaction.followup.send(f"✅ Зашёл на `{MC_SERVER}` как **{MC_USERNAME}**!", ephemeral=True); return
        await interaction.followup.send("⏳ Запущен, жду спавн...", ephemeral=True)

    # ── ROW 1 — Движение ─────────────────────────────────────────────────────
    @discord.ui.button(label="⬆️", style=discord.ButtonStyle.secondary, custom_id="mc_fwd",  row=1)
    async def btn_fwd(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("forward"); await interaction.response.send_message("⬆️", ephemeral=True)

    @discord.ui.button(label="⬇️", style=discord.ButtonStyle.secondary, custom_id="mc_back", row=1)
    async def btn_back(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("back"); await interaction.response.send_message("⬇️", ephemeral=True)

    @discord.ui.button(label="⬅️", style=discord.ButtonStyle.secondary, custom_id="mc_left", row=1)
    async def btn_left(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("left"); await interaction.response.send_message("⬅️", ephemeral=True)

    @discord.ui.button(label="➡️", style=discord.ButtonStyle.secondary, custom_id="mc_right", row=1)
    async def btn_right(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("right"); await interaction.response.send_message("➡️", ephemeral=True)

    @discord.ui.button(label="🐰 Jump", style=discord.ButtonStyle.secondary, custom_id="mc_jump_btn", row=1)
    async def btn_jump(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("jump", times=3); await interaction.response.send_message("🐰 x3", ephemeral=True)

    # ── ROW 2 — Экшны ────────────────────────────────────────────────────────
    @discord.ui.button(label="🏃 Run",    style=discord.ButtonStyle.primary,   custom_id="mc_run_btn",   row=2)
    async def btn_run(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("run"); await interaction.response.send_message("🏃 Бегу!", ephemeral=True)

    @discord.ui.button(label="🌀 Spin",   style=discord.ButtonStyle.primary,   custom_id="mc_spin_btn",  row=2)
    async def btn_spin(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("spin"); await interaction.response.send_message("🌀", ephemeral=True)

    @discord.ui.button(label="💃 Dance",  style=discord.ButtonStyle.primary,   custom_id="mc_dance_btn", row=2)
    async def btn_dance(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("dance"); await interaction.response.send_message("💃", ephemeral=True)

    @discord.ui.button(label="🥷 Sneak",  style=discord.ButtonStyle.secondary, custom_id="mc_sneak_btn", row=2)
    async def btn_sneak(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("sneak"); await interaction.response.send_message("🥷", ephemeral=True)

    @discord.ui.button(label="👋 Wave",   style=discord.ButtonStyle.secondary, custom_id="mc_wave_btn",  row=2)
    async def btn_wave(self, interaction, button):
        if not self._is_owner(interaction): return await self._deny(interaction)
        if not mc_bot.connected: return await interaction.response.send_message("❌ Бот не на сервере.", ephemeral=True)
        mc_bot.cmd("wave"); await interaction.response.send_message("👋", ephemeral=True)
...
async def ensure_mc_panel(channel):
    ...
    embed = discord.Embed(
        ...
        description=(
            "**Кнопки управления (только Owner):**\n\n"
            "🚀 **Запустить** — Aternos + зайти в MC\n"
            "🛑 **Остановить** — выйти + выключить Aternos\n"
            "📊 **Статус** — показать текущий статус\n"
            "🔌 **Подключить** — только зайти в MC (без Aternos)\n\n"
            "**Движение:**\n"
            "⬆️⬇️⬅️➡️ — ходьба  |  🐰 Jump\n\n"
            "**Экшны:**\n"
            "🏃 Run  🌀 Spin  💃 Dance  🥷 Sneak  👋 Wave\n\n"
            "**Текстовые команды:**\n"
            "`!say текст` — написать в чат MC\n"
            "`!jump [N]` — прыгнуть N раз\n"
            "`!mcstatus` — детальный статус\n"
            "`!mchelp` — список команд"
        ),
        color=0x2ecc71
    )
    mc_server_display = MC_SERVER or "*не задан (ATERNOS_USER/PASS + MC_SERVER в env)*"
...
async def on_message(message):
    ...
    if message.channel.id == MC_CHANNEL_ID:
        ...
        # !join [адрес] — зайти на сервер
            ...
            host = args_raw.strip() or MC_SERVER
            if not host:
                await message.channel.send("❌ `!join xxx.aternos.me`"); return
            m = await message.channel.send(f"🔌 Подключаюсь к `{host}`...")
            ok = mc_bot.start(host, MC_PORT_NUM, MC_USERNAME, MC_VERSION)
            if not ok:
                await m.edit(content="❌ Node.js не найден!"); return
            for _ in range(15):
                await asyncio.sleep(2)
                if mc_bot.connected:
                    await m.edit(content=f"✅ Зашёл на `{host}` как **{MC_USERNAME}**!"); return
            await m.edit(content="⏳ Подключился, жду спавн...")
            return
        ...
        # Команды движения/действий
        MC_ACTIONS = {
            "run":     ("run",     "🏃 Бегу!"),
            "forward": ("forward", "⬆️ Вперёд!"),
            "back":    ("back",    "⬇️ Назад!"),
            "left":    ("left",    "⬅️ Влево!"),
            "right":   ("right",   "➡️ Вправо!"),
            "spin":    ("spin",    "🌀 Кручусь!"),
            "dance":   ("dance",   "💃 Танцую!"),
            "sneak":   ("sneak",   "🥷 Крадусь..."),
            "wave":    ("wave",    "👋 Машу!"),
        ...
        if cmd == "jump":
            if not mc_bot.connected:
                await message.channel.send("❌ Бот не на сервере."); return
            try: n = int(args_raw.strip())
            except: n = 3
            mc_bot.cmd("jump", times=min(n, 10))
            await message.channel.send(f"🐰 Прыгаю {min(n,10)}x!")
            return

        if cmd == "say":
            if not mc_bot.connected:
                await message.channel.send("❌ Бот не на сервере."); return
            mc_bot.cmd("chat", text=args_raw or "Привет!")
            await message.channel.send(f"💬 Написал: `{args_raw[:80]}`")
            return

        if cmd in MC_ACTIONS:
            if not mc_bot.connected:
                await message.channel.send("❌ Бот не на сервере. Сначала `!join`"); return
            mc_cmd_key, ok_msg = MC_ACTIONS[cmd]
            mc_bot.cmd(mc_cmd_key)
            await message.channel.send(ok_msg)
            return
        ...
        if cmd in ("mchelp", "help"):
            await message.channel.send(
                ...
                "**Экшн:**\n"
                "`!jump [N]` `!spin` `!dance` `!sneak` `!wave`\n\n"
                "**Прочее:**\n"
                "`!say текст` — написать в чат MC\n"
                "`!mcstatus` — полный статус\n"
                "`!mchelp` — это сообщение"
            )
            return
